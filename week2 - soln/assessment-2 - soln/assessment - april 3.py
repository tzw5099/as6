...
def make_char_dict(filename):
    ...
    with open(filename) as f:
        for line_num, line in enumerate(f, 1):
            clean_line = line.strip(string.punctuation).lstrip().lower()
            output_dict[clean_line[0]].append(line_num)
    return output_dict 
...
def pandas_only_given_state(df, state):
    ...
    '''
    return df.loc[df['State'] == state,['Neighborhood', 'City', 'med_2011', 'med_2014']]
    # Another solution:
    # df.query('State == @state')[['Neighborhood', 'City', 'med_2011', 'med_2014']]

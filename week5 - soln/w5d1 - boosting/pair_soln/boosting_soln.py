...
class AdaBoostBinaryClassifier(object):
    ...
    def _boost(self, X, y, sample_weight):
        ...
        new_weights = sample_weight * np.exp(alpha*(predictions!=y))
    ...
    def predict(self, X):
        ...
        for estimator, alpha in zip(self.estimators_, self.estimator_weight_):
            pred_0_1 = estimator.predict(X) # estimator returns predictions as 0s and 1s
            pred_neg1_1 = 2*pred_0_1 - 1 # adaboost needs predictions as -1s and 1s
            weighted_vote_sum += alpha*pred_neg1_1
        return weighted_vote_sum > 0 # final output should be as 0s and 1s
...
if __name__ == '__main__':
    ...
    # part 1
    part_1(X_train, X_test, y_train, y_test)
    # part 2
    model_best = part_2(X_train, X_test, y_train, y_test)
    # part 3
    part_3(model_best, X_train)
